$(document).ready(function() {
    ...
    if ($(".write_container").data('type') === 'edit') {
        var allcat = $(".write_cat_select ul li");
        var catlist = $('.write_cat_select').data('catlist');
        var cats = catlist.split(',');
        for (var i = 0; i < cats.length; i++) {
            var newCat = cats[i].split('_')[1].toLowerCase();
            for (var j = 0; j < allcat.length; j++) {
                if (!allcat.eq(j).hasClass('select')) {
                    if (allcat.eq(j).html().toLowerCase() === newCat) {
                        allcat.eq(j).addClass('select');
                    }
                }
            }
        }
    ...
    $(".write_save").click(function(e) {
        ...
        var art_data = {
            ...
            type: $(".write_container").data('type'),
            urltitle: $(".write_container").data('urltitle')
        };
        $.ajax({
            type: 'POST',
